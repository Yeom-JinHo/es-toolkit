...
describe('safeJSONParse', () => {
  ...
  it('handles nested objects and arrays', () => {
    expect(safeJSONParse('{"user":{"name":"John","age":30},"tags":["admin","user"]}')).toEqual({
    ...
    expect(safeJSONParse('{"items":[{"id":1,"name":"item1"},{"id":2,"name":"item2"}]}')).toEqual({
      items: [
        { id: 1, name: 'item1' },
        { id: 2, name: 'item2' },
      ],
  ...
  it('uses the default generic when no type argument is provided', () => {
    const result = safeJSONParse('{"foo": 1}');
